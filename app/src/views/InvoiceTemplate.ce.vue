    ...
    <div class="template">
        <div class="header">
            <div>
                ...
                <div class="hide-in-print" style="margin-bottom:10px">This page is rendered from a "template" Vue.js
                    component.<br>You can modify the fields highlighted in green and interact with the buttons without
                    having them in the print. Check out <a
                        href="https://github.com/motla/vue-document-editor/blob/master/src/Demo/InvoiceTemplate.ce.vue"
                        target="_blank">InvoiceTemplate.ce.vue</a></div>
            </div>
            ...
            <tbody>
                <tr>
                    ...
                    <td style="min-width: 250px; text-align:right">
                        <div><b>Invoice number:</b> <span contenteditable="true" @keydown="preventLineBreaks">{{
                            modelValue.invoice_number }}</span></div>
                        <div><b>Invoice date:</b> <span contenteditable="true" @keydown="preventLineBreaks">{{ today
                        }}</span>
                        </div>
                        <div><b>Order number:</b> <span contenteditable="true"
                                @keydown="preventLineBreaks">W210984204</span></div>
                        <div><b>Order date:</b> <span contenteditable="true" @keydown="preventLineBreaks">{{ today }}</span>
                        </div>
                        <div><b>Your client number:</b> <span contenteditable="true"
                                @keydown="preventLineBreaks">120948</span></div>
                    </td>
                </tr>
            </tbody>
        </table>
        <table style="margin:10px 0 35px 0">
            ...
            <tbody>
                <tr>
                    <td ref="billing_address" contenteditable="true"
                        @input="(e) => this.billing_address = e.target.innerHTML"></td>
        ...
        <table style="margin-top:10px" class="order-table">
            ...
            <tbody>
                <tr v-for="(item, i) of items" :key="item.key">
                    <td>
                        <div class="left-btn hide-in-print" @click="() => items.splice(i, 1)">➖</div>{{ i + 1 }}
                    </td>
                    <td contenteditable="true">{{ item.ref }}</td>
                    <td contenteditable="true">{{ item.desc }}</td>
                    <td style="padding:0; text-align:center"><input type="number" min="1" style="width:3em"
                            v-model="item.qty"></td>
                ...
                <tr>
                    <td>
                        <div v-if="items.length < 5" class="left-btn hide-in-print"
                ...
                <tr>
                    <td colspan="6" style="text-align:right">Taxes: <span ref="tax_percent" contenteditable="true"
                            @input="(e) => this.tax_percent = parseFloat(e.target.innerText)"></span>%</td>
                </tr>
                <tr>
                    <td colspan="6" style="text-align:right">Shipping: € <span ref="shipping" contenteditable="true"
                            @input="(e) => this.shipping = parseFloat(e.target.innerText)"></span></td>
                </tr>
                <tr>
